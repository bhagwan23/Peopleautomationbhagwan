...
public class AddEarningComponentTest extends DriverFactory{
	...
	@Test
	@Description(value = "To verify that user is able to Add Earning Component ")
	public void addEarningComponent() throws InterruptedException {
		
		String coName = ReadPropertyUtil.readProperty("coName", confPath);
		//String dType = ReadPropertyUtil.readProperty("dType", confPath);

		AddEarningComponentPage ac = new AddEarningComponentPage(driver,wait);
		ac.clickComponentButton();
		System.out.println("printed");
		ac.clickAddComponentButton();
		ac.componentDetailsearning(coName);
		ac.saveButton();
		ac.verifySuccessMessageForEarning();
		ac.verifyAddedEarningComponent(coName);
